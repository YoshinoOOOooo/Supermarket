...
@ContextConfiguration(classes = {CheckoutController.class, GlobalExceptionHandler.class})
class CheckoutControllerTest {
    ...
    void calculateReturnsPublicQuoteWithTwoDecimalMoney() throws Exception {
        when(service.calculate(any())).thenReturn(new CheckoutResultView(
                Arrays.asList(
                        new CheckoutItemView("APPLE", "Apple", 2,
                                money("8.00"), money("16.00"), money("0.00"), money("16.00")),
                        new CheckoutItemView("STRAWBERRY", "Strawberry", 3,
                                money("13.00"), money("39.00"), money("0.00"), money("39.00")),
                        new CheckoutItemView("MANGO", "Mango", 1,
                                money("20.00"), money("20.00"), money("0.00"), money("20.00"))),
                money("75.00"), money("0.00"), money("75.00")));
        ...
        mvc.perform(post("/api/checkout/calculate")
                ...
                .andExpect(jsonPath("$.items[0].originalAmount").value("16.00"))
                .andExpect(jsonPath("$.items[0].discountAmount").value("0.00"))
                .andExpect(jsonPath("$.items[0].payableAmount").value("16.00"))
                .andExpect(jsonPath("$.originalAmount").value("75.00"))
                .andExpect(jsonPath("$.discountAmount").value("0.00"))
                .andExpect(jsonPath("$.payableAmount").value("75.00"));
